#requires -Modules AcmeSharp, Azure, AzureRM.Websites, AzureRM.Dns, AzureRM.Keyvault

<#PSScriptInfo
.VERSION 0.1
.TITLE Renew Cert - LetsEncrypt
.AUTHOR 
.GUID 
.DESCRIPTION 
.MANUAL 
...
###############################################################################################
## Lecture des informations du certificat dans le vault
###############################################################################################
$vaultCtx = Set-AzContext -Subscription "$VaultSubscription"

$ForceRenew = $false
if ( $CertNameInVault  -ne "" ) {
    $AllKeyVaultCerts = Get-AzKeyVaultCertificate -VaultName $VaultName | Where { $_.Name -eq "$CertNameInVault" } 
    $ForceRenew = $true
} 
else{
    $AllKeyVaultCerts = Get-AzKeyVaultCertificate -VaultName $VaultName
} 


$result = @()
$result += "CertNameInVault, KeyVaultCertEnabled, KeyVaultCertExpiryDate"
$createregistration=$true

foreach($KeyVaultCert in $AllKeyVaultCerts){

    $CertNameInVault = $KeyVaultCert.Name
    $KeyVaultCertEnabled = $KeyVaultCert.Enabled
    $KeyVaultCertExpiryDate = $KeyVaultCert.expires
    $Subject = (Get-AzKeyVaultCertificate -VaultName $VaultName -Name $CertNameInVault).Certificate.SubjectName.Name

    if($KeyVaultCertExpiryDate -lt ((Get-Date).AddDays($DayLimit)) -or $ForceRenew){
        $result+="$CertNameInVault, $KeyVaultCertEnabled, $KeyVaultCertExpiryDate"

        $CertTags = $KeyVaultCert.Tags
        if (! $CertTags -or $OnlyCheck ) { 
            if ( ! $CertTags ) { 
                "No Tags for $CertNameInVault ! "
            } 
        }
        else {
            ...
            $SubscriptionNameAll=$CertTags['subscription']
            $ResourceGroupAll=$CertTags['resource_group']
            $ResourceTypeAll=$CertTags['resource_type']
            $ResourcesAll=$CertTags['resources']
            $EndPoint_ListenerAll=$CertTags['endpoint_listener']
            $KeyVaultAll=$CertTags['keyvault']
            "Subject : $Subject"
            $DomainName = $Subject.Substring(3,$Subject.Length-3)
            "DomainName : $DomainName"
            $SanList=(Get-AzKeyVaultCertificatePolicy -VaultName $VaultName -Name $CertNameInVault).DnsNames | Where-Object { $_ -ne "$DomainName" }

            $null = Set-AzContext -Subscription "$DNSSubscriptionName"
            $dnsSubscriptionId=(Get-AzContext).Subscription.Id
            ...
            $identifiers = (Get-AzKeyVaultCertificatePolicy -VaultName $VaultName -Name $CertNameInVault).DnsNames | ForEach-Object { New-AcmeIdentifier $_ }
            ...
            if ($ChallengeOK) { 
                ...
                $null = Import-AzKeyVaultCertificate -VaultName $VaultName -Name $CertNameInVault -FilePath $pfxPath -Password $securePassword
                ...
                $null = Update-AzKeyVaultCertificate -VaultName $VaultName -Name $CertNameInVault -Tag $Tags
                ...
                ###############################################################################################
                $null = Set-AzContext -SubscriptionName $AzureContext.Subscription

                $params = @{"CertificateName"="$CertNameInVault";"DomainName"="$DomainName";"SubscriptionNameAll"="$SubscriptionNameAll";`
                ...
                Write-Error (("Certificate generation failed. Status is '{0}', can't continue as it is not 'valid'. " +
                    "Let's Encrypt could not retrieve the expected content from '$dnsName/$dnsValue'") -f $challenge.Status)
                $challenge
            }
        } 
    }
